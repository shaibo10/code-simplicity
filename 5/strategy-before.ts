  ...
  getCost(customerType, shippingData) {
    ...
    const shippingCost = shippingServicesUsed.map(service=>{
      ...
      if(customerType === 'managed'){
        return service.cost * 1.4 + companyExtraServicesUsedCost * 1.3 + minimumShippingCost
      }
      
      if(customerType === 'diy'){
        return service.cost * 1.5 + companyExtraServicesUsedCost * 2 + minimumShippingCost
      }
    })
